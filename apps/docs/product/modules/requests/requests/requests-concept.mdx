...
You can build custom workflows and steps. Avenue provides pre-built workflows for request operations in the `@avenuejs/requests` package.

For example, the `acceptProductRequestWorkflow`:

```ts title="src/workflows/requests/accept-product-request.ts"
import { WorkflowResponse, createWorkflow } from "@medusajs/workflows-sdk"
import { AcceptRequestDTO, updateProductStatusStep } from "@avenuejs/framework"
...
In this document, you'll learn about the main concepts related to requests in Avenue.
