...
You can build custom workflows and steps. Avenue provides pre-built workflows for wishlist operations in the `@avenuejs/b2c-core` package.
...
import { CreateWishlistDTO } from '@avenuejs/framework'
...
In this document, you'll learn about the main concepts related to wishlists in Avenue.
...
When retrieving wishlist items, Avenue automatically calculates current prices through the `calculateWishlistProductsPrice` utility:
