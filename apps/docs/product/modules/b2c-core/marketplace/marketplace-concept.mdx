...
You can build custom workflows and steps. Avenue provides pre-built workflows for marketplace operations in the `@avenuejs/b2c-core` package.
...
In this document, you'll learn about the main concepts related to order sets in Avenue.
...
When a customer checks out a cart containing products from multiple sellers, Avenue:
...
Avenue provides a `formatOrderSets` utility that aggregates information from individual orders to present unified order set data:
