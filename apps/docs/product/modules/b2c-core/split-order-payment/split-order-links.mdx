...
Avenue defines a link between the Order Module's `Order` data model and the Split Order Payment Module's `SplitOrderPayment` data model. This allows tracking payment amounts (authorized, captured, refunded) specific to each seller's portion of an order in a multi-vendor marketplace.
