...
The middleware `checkConfigurationRule` is used to restrict route availability according to the setting value, and can be found in `@avenuejs/b2c-core/src/shared/infra/http/middlewares/check-configuration-rule.ts`. It takes two arguments, `rule_type` and `expected_value`.
Advanced users can use it to modify the default Avenue behaviour.
