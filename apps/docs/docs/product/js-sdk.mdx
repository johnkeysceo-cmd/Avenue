---
title: "JS SDK"
description: "Learn how to use Medusa's JS SDK to interact with Avenue's marketplace APIs"
---

# JavaScript SDK

Avenue uses **Medusa's JavaScript SDK** to communicate with the backend APIs. The JS SDK provides a type-safe, promise-based interface for sending requests to Admin, Store, and Vendor APIs.

<Note>
  Avenue does not have a custom SDK. It uses Medusa's official JS SDK, which works seamlessly with all Avenue endpoints, including custom marketplace routes.
</Note>
...
The JS SDK provides methods for all Medusa core APIs. These work identically in Avenue.
...
## Using SDK with Avenue Endpoints

For Avenue's marketplace-specific APIs, use the `sdk.client.fetch` method. This is the standard approach used across all Avenue applications.
...
Avenue's Admin and Vendor panels use [Tanstack Query](https://tanstack.com/query/latest) extensively with the SDK for optimal caching and state management.
...
- **`sdk.client.fetch`** for Avenue-specific endpoints
