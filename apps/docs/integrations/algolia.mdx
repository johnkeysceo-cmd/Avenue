...
Avenue uses <a href="https://www.algolia.com/">Algolia</a> as a search engine to boost product search accuracy and serving trending/recommended items. The integration features auto product indexing, whenever any of the product attributes was changed or a new product review is created.
...
> **Important:** The settings below are configured for the **default Avenue storefront**. If you're using a custom storefront with different filtering requirements, you'll need to adjust `attributesForFaceting` based on your specific filters. See the [Custom Storefront Configuration](#custom-storefront-configuration) section below.
...
The default Avenue storefront uses the following filters in product listing pages:

**Repository:** [avenuejs/b2c-marketplace-storefront](https://github.com/avenuejs/b2c-marketplace-storefront)
...
👉 **[View and download algolia-config.json](https://github.com/avenuejs/b2c-marketplace-storefront/blob/main/algolia-config.json)**
...
1. **Via Algolia Dashboard (recommended):**
   - Download the [algolia-config.json](https://github.com/avenuejs/b2c-marketplace-storefront/blob/main/algolia-config.json) file
...
For a product to be visible on the **default Avenue storefront** via Algolia search, it must meet **all** of the following conditions:
