...
description: What changed and how to port your existing Avenue 1.x code to 2.0.
---
...
|-------------|----------------|
| `@avenuejs/b2c-core` | `@avenuejs/core-plugin` |
| `@avenuejs/commission` | Built into core-plugin |
| `@avenuejs/algolia` | Block: `avenuejs add algolia` |
...
| `@medusajs/js-sdk` | `@avenuejs/client` |
...
Copy to `packages/api/src/modules/`, register in `medusa-config.ts`. Update imports from `@avenuejs/b2c-core` to `@avenuejs/core-plugin`.
...
Run `bunx @avenuejs/cli@latest codegen` after adding routes.
...
|-----------|------------|
| `@medusajs/js-sdk` | `@avenuejs/client` |
| `@custom-types/*` | `@avenuejs/types` |
| `@hooks/*`, `@components/*`, `@lib/*` | Keep local, or `@avenuejs/dashboard-shared` if the symbol exists there |
