...
Both the admin panel and vendor portal use the same SDK (`@avenuejs/dashboard-sdk`). Customization is file-based and convention-driven — you add pages by creating files, configure navigation through exports, and override components through config.
...
import { dashboardPlugin } from '@avenuejs/dashboard-sdk'
...
```typescript avenue.config.ts
import { defineConfig } from '@avenuejs/dashboard-sdk'
...
Set `name` and `logo` in `avenue.config.ts` to customize the sidebar header:
...
Override built-in layout components in `avenue.config.ts`:
