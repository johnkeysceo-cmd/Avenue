...
The Avenue CLI (`@avenuejs/cli@latest`) manages your marketplace project — creating projects, installing blocks, generating types, and checking for updates.

## Installation

```bash
bun add -g @avenuejs/cli@latest
```
...
Scaffold a new Avenue project from a template.
...
```bash
bunx @avenuejs/cli@latest add wishlist
bunx @avenuejs/cli@latest add product-review order-tracking
```
...
```bash
bunx @avenuejs/cli@latest search --query wishlist
```

| Option | Description |
|--------|-------------|
| `-q, --query <query>` | Search by name or description |
| `-r, --registry <registry>` | Registry to search (default: `@avenuejs`) |
...
```bash
bunx @avenuejs/cli@latest view wishlist
```
...
```bash
bunx @avenuejs/cli@latest diff wishlist
```

If there are changes you want, update with:

```bash
bunx @avenuejs/cli@latest add wishlist --overwrite
```
...
```bash
bunx @avenuejs/cli@latest codegen
```
...
```bash
bunx @avenuejs/cli@latest build
```
...
```bash
bunx @avenuejs/cli@latest telemetry --disable
bunx @avenuejs/cli@latest telemetry --enable
```
...
{
  "registries": {
    "@avenuejs": "https://registry.avenuejs.com/{name}",
...
```bash
bunx @avenuejs/cli@latest search --query review --registry @my-registry
```
