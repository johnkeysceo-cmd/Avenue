...
A registry is a collection of blocks that can be installed into any Avenue project using the CLI. The official Avenue registry (`@avenuejs`) provides all the built-in marketplace blocks. You can also create and host your own.

## How it works

A registry is a set of JSON files — one per block — served over HTTP. When you run `bunx @avenuejs/cli@latest add`, the CLI fetches the block's JSON, resolves its dependencies, and copies the source files into your project.
...
| ---------------------- | -------------------------------------------------------------- |
| `name`                 | Block identifier used in `bunx @avenuejs/cli@latest add`               |
| `description`          | Short description shown in search results                      |
| `dependencies`         | NPM packages to install                                        |
| `registryDependencies` | Other blocks this block depends on (e.g. `@avenuejs/wishlist`) |
...
Add your registry to `blocks.json` in any Avenue project:
...
```bash
bunx @avenuejs/cli@latest add @my-org/reviews
```
...
{
  "name": "order-tracking",
  "registryDependencies": ["@avenuejs/wishlist"],
  "files": [...]
}
```
