...
Blocks are the unit of distribution in Avenue. Each block is a self-contained feature — a reviews system, a wishlist, product import/export — that you install directly into your project. Unlike packages, blocks are copied as source code. You own every file and can modify anything.

## Installing blocks

```bash
bunx @avenuejs/cli@latest add reviews
bunx @avenuejs/cli@latest add wishlist product-import-export
```
...
4. **Regenerate types** with `bunx @avenuejs/cli@latest codegen`
...
```bash
bunx @avenuejs/cli@latest diff reviews
```

If there are updates you want, re-install with the overwrite flag:

```bash
bunx @avenuejs/cli@latest add reviews --overwrite
```
...
```bash
bunx @avenuejs/cli@latest search
...
```bash
bunx @avenuejs/cli@latest view reviews
```
