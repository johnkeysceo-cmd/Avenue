---
title: "API Client"
description: "Type-safe client for consuming Avenue APIs."
---

`@avenuejs/client` provides a type-safe way to interact with your Avenue backend. Every endpoint, request input, and response is fully typed — inferred directly from your backend routes.

## Installation

```bash
bun add @avenuejs/client
```

## Create a client

```typescript
import { createClient } from "@avenuejs/client";
import type { Routes } from "./.avenue/_generated";
...
<Info>
  The `Routes` type is generated from your backend using `@avenuejs/cli@latest
...
```typescript
import type { InferClientInput, InferClientOutput } from "@avenuejs/client";
