---
title: "Skills"
description: "Pre-built AI workflows that ship with every Avenue project for safe, guided development."
---

# Skills

Every Avenue project created with `@avenuejs/cli@latest create` includes a set of **skills** — reusable prompt modules that teach your AI coding tool how to work with Avenue correctly.

Skills encode domain knowledge, enforce patterns, and catch common mistakes. Instead of prompting your AI with "please follow the Avenue conventions for forms", the skill handles it automatically.
...
Each skill contains rules, checklists, and workflows tailored to a specific aspect of Avenue development.

---

## Included skills

Every project created from the Avenue starter template includes these skills:

| Skill | What it does | When it triggers |
|-------|-------------|-----------------|
| `avenue-cli` | Guides correct usage of CLI commands (`create`, `init`, `add`, `search`, `view`, `diff`) | Working with the Avenue CLI |
| `avenue-blocks` | Discovers, evaluates, installs, and verifies blocks using `blocks.json` aliases | Adding or updating blocks |
| `migration-guide` | Analyzes a 1.x project and walks through migration to 2.0 step by step | Upgrading from Avenue 1.x |
...
Claude Code matches this to the `avenue-blocks` skill and activates it automatically.
