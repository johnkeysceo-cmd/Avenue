---
name: avenue-blocks
description: Discover, evaluate, install, and verify Avenue blocks safely using `blocks.json` aliases and Avenue CLI workflows. Use when adding or updating blocks in a Avenue project.
argument-hint: "[block-name]"
---

# Avenue Blocks Workflow

Use this skill when:
- evaluating whether a block fits a project
- adding a block through Avenue CLI
...
- the Avenue CLI command that matches the task: `search`, `view`, `add`, or `diff`
...
```bash
npx @avenuejs/cli@canary search --query <keyword>
npx @avenuejs/cli@canary view <block>
npx @avenuejs/cli@canary add <block>
npx @avenuejs/cli@canary diff <block>
```
