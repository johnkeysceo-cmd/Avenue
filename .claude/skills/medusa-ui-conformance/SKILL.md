...
description: Keep custom admin and vendor UI aligned with @medusajs/ui and Radix UI. Use when adding or modifying reusable UI, page-level interaction patterns, overlays, menus, form primitives, or any custom component in Avenue dashboards.
---
...
This skill is the system-level gate for Avenue dashboard UI.
...
5. Do not hardcode visual language outside Avenue and Medusa conventions. Prefer Medusa UI components, tokens, spacing, and composition.
...
Prefer existing Avenue wrappers when the pattern already exists.
...
Avenue already uses `asChild` heavily. When composing with Medusa UI or Radix:
...
- give it an API that feels consistent with Medusa UI and Avenue wrappers
