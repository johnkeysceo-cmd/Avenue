...
description: Plan and execute migration from Avenue 1.x to 2.0. Classifies project difficulty, reads relevant migration docs, and follows stop conditions.
...
Use this skill when:
- Planning a migration from Avenue 1.x to 2.0
...
1. `docs/migrations/avenue-1.x-to-2.0/README.md` — entry point, version check, package/directory mapping, approach
...
1. Inventory the source project:
   - Read `package.json` — note `@avenuejs/b2c-core` version to determine 1.x version
...
Based on classification, read from `docs/migrations/avenue-1.x-to-2.0/`:
...
3. Add registry blocks (`bunx @avenuejs/cli@latest add <block>`)
...
- A custom module depends on internal APIs not exposed in `@avenuejs/core-plugin`
