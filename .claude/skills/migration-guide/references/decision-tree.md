...
Does the 1.x project have custom MedusaJS modules (not from `@avenuejs/b2c-core` or standard packages)?
...
How many custom workflows exist (beyond those provided by Avenue packages)?
...
- **Yes** (Stripe Connect, Resend, TalkJS, custom providers) → elevate to at least **light-custom**, check `docs/migrations/avenue-1.x-to-2.0/README.md` for the known-gaps section
...
- **Custom storefront only, no backend changes**: Still **starter** for Avenue migration (storefront is out of scope)
