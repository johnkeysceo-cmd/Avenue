...
```
/Users/jakubsieradzki/workspace/testing-avenue/avenue-testing-registry/apps/admin/src/pages/
```

These pages are file-based routed (same as main admin app via `@avenuejs/dashboard-sdk`).

## Purpose

Consumer pages test that CC compound components are properly composable from an external consumer perspective. They import from `@avenuejs/admin/pages` and customize layout by overriding children.
...
```typescript
import { ProductDetailPage } from "@avenuejs/admin/pages"
...
```typescript
import { ProductListPage } from "@avenuejs/admin/pages"
...
import * as AdminPages from "@avenuejs/admin/pages"
