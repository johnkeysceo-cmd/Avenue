  ...
  routes: [
    ...
    ...hooksRoutesMiddlewares,
    {
      ...
      middlewares: [(req: MedusaRequest, res: MedusaResponse, next: MedusaNextFunction) => {
        const adminUI = req.scope.resolve<AdminUIModuleService>(AvenueModules.ADMIN_UI)
        const vendorUI = req.scope.resolve<VendorUIModuleService>(AvenueModules.VENDOR_UI)
