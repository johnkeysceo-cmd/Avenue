...
import type { AvenueConfig, BuiltAvenueConfig } from "./types";
...
export function avenueDashboardPlugin(pluginConfig: AvenueConfig): Vite.Plugin {
    let root: string;
    let config: BuiltAvenueConfig;

    return {
        name: "@avenuejs/dashboard-sdk",
        async config(viteConfig) {
            ...
            return {
                ...
                optimizeDeps: {
                    exclude: [
                        "virtual:avenue/config",
                        "virtual:avenue/routes",
                        "virtual:avenue/components",
                        "virtual:avenue/menu-items",
                        "virtual:avenue/i18n",
                    ],
                    include: [
                        "react",
        ...
        load(id) {
            return loadVirtualModule({ cwd: root, id, avenueConfig: config });
        },
