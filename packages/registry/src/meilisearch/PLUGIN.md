# Meilisearch Search Block for Avenue

## What It Does

The Meilisearch Search block adds fast, full-text product search to your Avenue marketplace. Shoppers can find products instantly by typing keywords, filtering by category, price range, or seller — all with results appearing in milliseconds.
...
Meilisearch is an open-source search engine. You need a running instance accessible from your Avenue server.
...
### Avenue Marketplace
A running Avenue marketplace with products and sellers already configured.
...
From your Avenue project root, run:

```bash
npx @avenuejs/cli add meilisearch
```
...
**Is there a cost?**
Meilisearch itself is open source and free to self-host. Meilisearch Cloud offers managed hosting with paid plans. The Avenue block is free.
