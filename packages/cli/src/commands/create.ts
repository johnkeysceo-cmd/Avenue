...
export const create = new Command()
  .name("create")
  .description("create a new Avenue project")
    ...
    try {
      ...
      if (!projectName) {
        const { enteredName } = await prompts({
          ...
          initial: opts.template ? `${opts.template}-app` : "my-avenue",
      ...
      spinner(`Avenue project successfully created! (${totalDuration}s)`).succeed();
...
async function downloadTemplate({
  ...
  const url = `https://codeload.github.com/avenuejs/avenue/tar.gz/${DEFAULT_BRANCH}`;
  const templatePath = CREATE_TEMPLATES[template].path;
  const filter = `avenue-${DEFAULT_BRANCH.replace(/^v/, "").replaceAll("/", "-")}/templates/${templatePath}/`;
...
${header("Documentation:")}
  ...
  - ${createTerminalLink("Getting Started", "https://docs.avenuejs.com/v2/welcome")}
`;
}
...
function validateNodeVersion(): void {
  ...
  if (nodeVersion < MIN_SUPPORTED_NODE_VERSION) {
    throw new Error(
      `Avenue requires at least v${MIN_SUPPORTED_NODE_VERSION} of Node.js. You're using v${nodeVersion}. Please install at least v${MIN_SUPPORTED_NODE_VERSION} and try again: https://nodejs.org/en/download`
    );
  }
}
