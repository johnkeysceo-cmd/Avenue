  ...
  .action(async (opts) => {
    try {
      ...
      if (!opts.enable && !opts.disable) {
        ...
        const envDisabled = process.env.AVENUE_DISABLE_TELEMETRY === 'true';
        const enabled = configEnabled && !envDisabled;
        const status = enabled ? "enabled" : "disabled";
        logger.log(`Telemetry is currently ${status}.`);
        if (envDisabled && configEnabled) {
          logger.log(`(Disabled via AVENUE_DISABLE_TELEMETRY environment variable)`);
        }
        logger.log(
          enabled
            ? `Run "avenuejs telemetry --disable" to opt out.`
            : `Run "avenuejs telemetry --enable" to opt in.`
        );
