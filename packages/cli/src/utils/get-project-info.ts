...
export async function getProjectInfo(cwd: string): Promise<ProjectInfo> {
  const [isSrcDir, aliasPrefix, packageJson, medusaConfigInfo, medusaVersion, avenueVersion] = await Promise.all([
    fs.pathExists(path.resolve(cwd, "src")),
    getTsConfigAliasPrefix(cwd),
    getPackageInfo(cwd),
    getMedusaConfigFile(cwd),
    getMedusaVersion(cwd),
    getAvenueVersion(cwd),
  ]);
...
export async function getAvenueVersion(cwd: string): Promise<string | null> {
  const packageJson = getPackageInfo(cwd, false)

  const rootVersion = packageJson?.dependencies?.['@avenuejs/cli']
  if (rootVersion) {
    return rootVersion
  }
  ...
  for (const file of packageJsonFiles) {
    const pkgJson = getPackageInfo(path.join(cwd, path.dirname(file)), false)
    const version = pkgJson?.dependencies?.['@avenuejs/cli']
    if (version) {
      return version
    }
  }
