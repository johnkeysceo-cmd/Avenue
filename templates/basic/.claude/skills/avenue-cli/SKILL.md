---
name: avenue-cli
description: Use Avenue CLI commands correctly inside a project created from the Avenue basic starter. Use when choosing between `create`, `init`, `add`, `search`, `view`, and `diff`.
---
...
- `create` — create a new Avenue project
...
- Run Avenue CLI registry commands (`search`, `view`, `add`, `diff`) from the **project root** where `blocks.json` lives.
...
npx @avenuejs/cli@latest view <block-name>
```

### Add a block

```bash
npx @avenuejs/cli@latest add <block-name>
```
...
```bash
npx @avenuejs/cli@latest diff <block-name>
```
