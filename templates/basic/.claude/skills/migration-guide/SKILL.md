...
description: Analyze a Avenue 1.x project and guide migration to 2.0. Self-contained — works without access to the avenue monorepo.
argument-hint: "[path-to-old-project]"
---

# Migration Guide: Avenue 1.x → 2.0

Use this skill when:
- Migrating an existing Avenue 1.x project to 2.0
...
| 1.x | 2.0 |
|-------|--------|
| `@avenuejs/b2c-core` | `@avenuejs/core-plugin` — all core modules built in |
| `@avenuejs/commission` | Built into core-plugin |
| `@avenuejs/algolia` | Registry block: `avenuejs add algolia` |
| `@avenuejs/resend` | No 2.0 equivalent — port manually |
| `@avenuejs/payment-stripe-connect` | No 2.0 equivalent — port manually |
| `@avenuejs/stripe-tax-provider` | No 2.0 equivalent — port manually |
| `@medusajs/admin-vite-plugin` | `@avenuejs/dashboard-sdk` |
| `@medusajs/js-sdk` | `@avenuejs/client` (generated typed client) |
...
| `from "@medusajs/js-sdk"` | `from "@avenuejs/client"` |
| `from "@custom-types/*"` | `from "@avenuejs/types"` |
| `from "@hooks/*"` | `from "@avenuejs/dashboard-shared"` or local |
| `from "@components/*"` | `from "@avenuejs/dashboard-shared"` |
| `from "@lib/*"` | `from "@avenuejs/dashboard-shared"` |
| `from "@avenuejs/b2c-core"` | `from "@avenuejs/core-plugin"` |
...
```typescript
// 1.x
plugins: [{ resolve: "@avenuejs/b2c-core", options: {} }]

// 2.0
plugins: [{ resolve: "@avenuejs/core-plugin", options: {} }]
...
- Read `package.json` — list all `@avenuejs/*` and `@medusajs/*` dependencies. Note the `@avenuejs/b2c-core` version to determine project version.
...
- Has registry block? → install with `avenuejs add <block>`
...
After API routes: run `bunx @avenuejs/cli@latest codegen`.
...
Full migration documentation: https://docs.avenue.js/v2/migrations/overview
