...
description: Keep custom admin and vendor UI in the Avenue basic starter aligned with local wrappers and @medusajs/ui. Use when adding reusable UI, interactive primitives, overlays, menus, selectors, or custom dashboard components.
---
...
Use this order:
1. `@avenuejs/dashboard-shared` components (TabbedForm, _DataTable, SingleColumnPage, TwoColumnPage, RouteFocusModal, Form, etc.)
...
## Available from @avenuejs/dashboard-shared
...
2. Do not build custom tabbed forms — use `TabbedForm` from `@avenuejs/dashboard-shared`.
3. Do not build custom data tables — use `_DataTable` from `@avenuejs/dashboard-shared`.
...
- does `@avenuejs/dashboard-shared` export a component for this?
...
- reused `@avenuejs/dashboard-shared` where applicable
