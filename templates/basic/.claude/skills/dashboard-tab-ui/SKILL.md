...
description: Create or modify tabbed workflows in admin or vendor dashboards in the Avenue basic starter using stable tab structure, validation scope, and i18n-aware wizard conventions.
---
...
Always use `TabbedForm` from `@avenuejs/dashboard-shared` — do NOT build custom tab navigation with `ProgressTabs` from `@medusajs/ui` directly.
...
- Use `Form.Field` / `Form.Item` / `Form.Label` / `Form.Control` / `Form.ErrorMessage` from `@avenuejs/dashboard-shared`.
