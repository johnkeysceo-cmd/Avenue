...
description: Create or modify pages in admin or vendor dashboards in the Avenue basic starter using correct routing, page composition, i18n, and dashboard UI conventions.
---
...
- Route files belong under `src/routes` (NOT `src/pages` — the `avenueDashboardPlugin` scans `src/routes` only).
...
6. Do not place route files in `src/pages` — the `avenueDashboardPlugin` scans `src/routes` only.
...
```tsx
import type { RouteConfig } from "@avenuejs/dashboard-sdk";
...
Use `_DataTable`, `SingleColumnPage`, and `useDataTable` from `@avenuejs/dashboard-shared`.
...
Detail pages use `TwoColumnPage` with `.Main` and `.Sidebar` from `@avenuejs/dashboard-shared`.
